            ...
            if (success && success !== null) {
                ...
                room.addEventListener('active-talkers-updated', function (event) {
                    ...
                    if (event.message && event.message !== null && event.message.activeList && event.message.activeList !== null) {
                        ...
                        if (ATList.length === 0 && document.querySelectorAll('.remote-view').length === 0) {
                            confo_variables.previousToggleClass = '';
                    ...
                    ATList.forEach((item, index) => {
                        if (item.clientId === confo_variables.SpotLightClientId && item.spotlight === false) {
                            ...
                            toastr.info(`${item.name} is spotlight`);
                        }
                    });
                ...
                room.addEventListener('fs-download-result', function (event) {
                    ...
                    switch (evtMsg.messageType) {
                        case 'download-available':
                            var filename = evtMsg.response.downloadInfo.name;
                            // var createAt = evtMsg.response.downloadInfo.createdAt;
                            var sender = evtMsg.response.downloadInfo.sender; // sender name from SDK
                            // var broadCast = evtMsg.response.downloadInfo.broadcast;
                            var senderId = evtMsg.response.downloadInfo.senderId;
                            var job_id = evtMsg.response.jobId;

                            // var obj = {
                            //     'msg': `${sender} shared a file <br/> File: ${filename}<br/> <a href="#" style="font-size:inherit;" onclick="file_download(${job_id})">[Download]</span>`,
                            ...
                            var head = document.createElement('div');
                            head.setAttribute('class', 'head');
                            head.innerHTML = `<p>${sender}</p>`;
                            desc.appendChild(head);
                            var message = document.createElement('div');
                            message.setAttribute('class', 'message');
                            message.innerHTML = `<p>${sender} shared a file <br/> File: ${filename}<br/> <a href="#" style="font-size:inherit;" onclick="file_download(${job_id})">[Download]</span>}</p>`;
                            desc.appendChild(message);
                            var time_div = document.createElement('div');
                            time_div.setAttribute('class', 'time');
                            time_div.setAttribute('style', 'font-size: smaller');
    ...
    startRecord: function () {
        room.startRecord(function (result, error) {
            ...
            if (result.result == 0) {
                ...
                toastr.options.positionClass = 'toast-bottom-right';
                toastr.error("Room recording not started");
            }
        });
    },
    stopRecord: function () {
    ...
    spot_light: function (param) {
        ...
        room.addSpotlightUsers([confo_variables.activeTalkerInfo[parseInt(str_id)].clientId], function (resp) {
            ...
            if (resp.result.result === 0) {
                ...
                toastr.error("Spotlight didn't work");
            }
        })
    },
    spotlightRemove: function (param) {
        ...
        room.removeSpotlightUsers([confo_variables.activeTalkerInfo[parseInt(str_id)].clientId], function (resp) {
            if (resp.result.result === 0) {
                ...
                toastr.options.positionClass = 'toast-bottom-right';
                toastr.options.timout = '10000000';
                toastr.error("Remove Spotlight didn't work");
            }
...
EnxRtc.getDevices(function (arg) {
    ...
    if (arg.result === 0) {
        arg.devices.cam.forEach(element => {
            ...
            if (element.deviceId !== '') {
                camlist += `<div class="devices"><input type="radio" id="${element.deviceId}" name="camera" value="${element.label}" onclick="switchcam(this)"> <label for="${element.deviceId}">${element.label}</label></div>`
            }
        });
        ...
        arg.devices.mic.forEach(element => {
            ...
            if (element.deviceId !== '') {
                miclist += `<div class="devices"><input type="radio" id="${element.deviceId}" name="mic" value="${element.label}" onclick="switchmic(this)"> <label for="${element.deviceId}">${element.label}</label></div>`
            }
        });
...
if (document.getElementById('file_upload_public')) {
...
function handlePublicFileSharing(evt) {
    ...
    document.querySelector('textarea').value = `You shared a file <br/> File: ${filename}`;
    chatSend('file');
...
function file_download(id) {
    ...
    room.recvFiles(availableFiles[id].index, { isMobile: true },
        function callbacker(res) {
            ...
            reader.onloadend = function () {
                ...
                let fileName = res.response.name;
...
function chatSend(obj) {
    confo_variables.chatSendToOthers(obj);
}
